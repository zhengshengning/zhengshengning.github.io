...
其中第二项是 Attention 的计算量（每层每个头有两个 $(s, d_k) \times (d_k, s)$ 的矩阵乘法，每次贡献 $2s^2 d_k$，共 $4s^2 d_k$）。当序列长度 $s$ 较短时（比如 2048），第二项远小于第一项，"$2Ns$" 是一个好的近似。当 $s$ 很长（如 128K）时，Attention 的计算量可能接近甚至超过线性层。
