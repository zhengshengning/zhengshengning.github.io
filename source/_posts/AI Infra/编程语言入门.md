...
基础打好之后，你可以根据兴趣和职业方向选择深入的路径：

| 方向 | 核心内容 | 推荐入口 |
|------|---------|---------|
| CUDA 编程与算子优化 | GPU 架构、Kernel 编写、FlashAttention | [NVIDIA CUDA Programming Guide](https://docs.nvidia.com/cuda/cuda-c-programming-guide/) |
| 分布式训练 | 数据并行、3D 并行、ZeRO、混合精度 | [PyTorch DDP 教程](https://pytorch.org/tutorials/intermediate/ddp_tutorial.html) |
| 推理与部署 | KV Cache、PagedAttention、量化、Speculative Decoding | [vLLM 官方文档](https://docs.vllm.ai/) |
| Transformer 架构深入 | Self-Attention、FFN、位置编码、手写 GPT | [Andrej Karpathy: Let's build GPT from scratch](https://www.youtube.com/watch?v=kCc8FmEb1nY) |
